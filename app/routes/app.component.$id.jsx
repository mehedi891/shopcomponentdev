...
const UpdateComponent = () => {
  ...
  const [showCopyCodeBanner, setShowCopyCodeBanner] = useState(true);
  ...
  const { setError, getValues, handleSubmit, reset, formState: { errors, isDirty }, control, watch, setValue } = useForm({
    defaultValues: {
      ...
      appliesTo: component?.appliesTo || APPLIES_TO.byproduct,
      addToCartType: {
        type: component?.addToCartType?.type || ADD_TO_CART_TYPE.individual,
        products: component?.addToCartType?.products || [],
      },
      enableQtyField: component?.enableQtyField || false,
      layout: component?.layout || LAYOUT.grid,
      status: component?.status || STATUS.activate,
      componentSettings: {
        fullView: component?.componentSettings?.fullView || false,
        cartBehavior: component?.componentSettings?.cartBehavior || CART_BEHAVIOR.checkout,
        customCss: component?.componentSettings?.customCss || '',
        showComponentTitle: component?.componentSettings?.showComponentTitle || SHOW_COMPONENT_TITLE.yes
      },
  ...
  useEffect(() => {
    const isShowBanner = localStorage.getItem('scpCopyBannerShow');
  ...
  useEffect(() => {
    ...
    if (component?.appliesTo === APPLIES_TO.byproduct) {
      if (component?.addToCartType?.type === ADD_TO_CART_TYPE.individual) {

        setSelectedProductsInd(limitedProducts);

      } else if (component?.addToCartType?.type === ADD_TO_CART_TYPE.bulk) {
        setSelectedProductsBulk(limitedProducts);
      }
    } else if (component?.appliesTo === APPLIES_TO.bycollection) {
        ...
        >
          <s-query-container>
            ...
            >
              ...
              <s-text type="strong">{"Updated Component: " + component?.title}</s-text>
            </s-stack>

            {showNewCreatedBanner &&

              <s-banner
                heading={'Component Created Successfully'}
                tone="success"
                onDismiss={() => {
                  setShowNewCreatedBanner(false);
                }}
              >
                ...
                <s-stack direction='inline' gap={'base'} alignItems="center" justifyContent="start">
                  <s-link href={`/app/createcomponent`}>Create another component</s-link>
                  <s-link href={`/app`}>Dashboard</s-link>
                  <s-button variant="tertiary" disabled={isDirty || watchedValues.status === STATUS.deactivate} onClick={handleCopyHtmlCode}>Copy code</s-button>
            ...
            >
              ...
              <s-grid-item gridColumn="span 4">
                ...
                <s-stack paddingBlockEnd="base" direction="inline" justifyContent="end" alignItems="center" gap='small-200'>
                  <s-button variant="secondary" type='reset' onClick={() => {
                    handleDiscard();
                  }}
                    disabled={!isDirty}>
                    Discard
                  </s-button>
                  <s-button variant="primary" loading={navigation.state === 'submitting'} disabled={!isDirty} type='submit'>Update</s-button>
                ...
                <div className="previewSectionSticky">
                  {showCopyCodeBanner &&
                    <s-box paddingBlockEnd='small-200'>
                      ...
                      >
                        <s-box>
                          <s-text>Done configuring? Copy code → Paste into an HTML/Custom Code block (where you want to show the component) → Publish. Your component is live.
                          </s-text>
                        </s-box>
                    ...
                    >
                      ...
                      >
                        ...
                        <s-stack direction="inline" gap="small-200" alignItems="center">
                          <s-icon type='info' tone='info' interestFor='showInstructionTooltip' />
                          <s-tooltip id="showInstructionTooltip">Done configuring? Copy code → Paste into an HTML/Custom Code block (where you want to show the component) → Publish. Your component is live.</s-tooltip>
                          <s-button variant="primary" onClick={handleCopyHtmlCode} accessibilityLabel="Copy code" disabled={isDirty || watchedValues.status === STATUS.deactivate}>Copy code</s-button>
                        </s-stack>
                      </s-stack>
